...
class mslib_fe
{
		...
		foreach ($ms_menu as $tablevel1_key => $tablevel1) {
			...
			else
			{
				...
				else
				{
					...
					{
						$counter_tablevel2++;
						if ($type=='header' and ($counter_tablevel2==$total_tablevel2)) $tablevel2_params='dropdown_bottom';
						elseif ($type=='footer' and ($counter_tablevel2==1)) 			$tablevel2_params='dropdown_top';
						else $tablevel2_params='';			
						...
						{					
							...
							{
								$counter_tablevel3++;
								if ($type=='header' and ($counter_tablevel3==$total_tablevel3)) $tablevel3_params='dropdown_bottom';
								elseif ($type=='footer' and ($counter_tablevel3==1)) 			$tablevel3_params='dropdown_top';
								else $tablevel3_params='';											
	...
	function final_attributes_price ($product, $attributes, $quantity=1,$add_currency=0,$ignore_minimum_quantity=0)
	{
		...
		if ($add_currency)	return mslib_fe::amount2Cents2($final_price);
				...
				foreach ($records as $record)
				{
						...
						if ($item[0] !='keyword' and $item[0] !='label')
						{
								if (strstr($item[1],","))
								{
										...
										if (count($selected) >1) foreach ($selected as $select) $this->ms['parameters'][$item[0]][]=$select;
										else $this->ms['parameters'][$item[0]]=$item[1];
								}
								else $this->ms['parameters'][$item[0]]=$item[1];
						}
						else $this->ms['parameters'][$item[0]]=$item[1];
			...
			while (list($key, $value) = each($this->get))
			{
				if (!mslib_fe::tep_in_array($key, $exclude_array))
				{
					if (!is_array($value))
					{
						...
						{
							if ($hidden_fields) $get_url .= '<input name="'.$key.'" type="hidden" value="'.rawurlencode(stripslashes($value)).'">'."\n";
							else $get_url .= $key . '=' . rawurlencode(stripslashes($value)) . '&';
						}
					}
					else
					{
						foreach ($value as $$key => $$value)
						{
							...
							{							
								if (!is_array($$value))
								{
									...
									{
										if ($hidden_fields) $get_url .= '<input name="'.$key . rawurlencode('['.$$key.']').'" type="hidden" value="'.rawurlencode(stripslashes($$value)).'">'."\n";
										else $get_url .= $key . rawurlencode('['.$$key.']') . '=' . rawurlencode(stripslashes($$value)) . '&';					
									}
								}
								else
								{
									foreach ($$value as $k => $v)
									{
										...
										{
											if ($hidden_fields) $get_url .= '<input name="'.$key . rawurlencode('['.$$key.'][]').'" type="hidden" value="'.rawurlencode(stripslashes($v)).'">'."\n";
											else $get_url .= $key . rawurlencode('['.$$key.'][]') . '=' . rawurlencode(stripslashes($v)) . '&';
										}
									}
								}
							}
	...
	function getUser ($value, $field='uid') {
		if ($value) {
			if ($field=='code') $field='tx_multishop_code';
			if ($field) {
	...
	{
		if ($value == $this->conf['fe_customer_usergroup'] or $value == $this->conf['fe_admin_usergroup'] or $value == $this->conf['fe_rootadmin_usergroup']) return false;		
		if ($value)
		{
			if ($type=='uid')
			{
	...
	function getUsers ($groupid,$orderby='company',$include_disabled=0)
	{
		if (is_numeric($groupid) and $groupid >0)
		{
			...
			if (!$this->masterShop)
			{
				$filter[]="page_uid='".$this->shop_pid."' and ";	
			}
	...
	{
		if (is_numeric($usergroup)) $admin_group=$usergroup;
		else die("no admin group defined yet. Please add the admin usergroup to the constants field in the TYPO3 template.");
	...
	function rewritenamein($input,$id='')
	{
		...
		$input = trim($input,'/');
		$input = trim($input);
		if ($id)	$final_file=$input.'-'.$id;
		else		$final_file=$input;
	...
	function orderhasitems ($orders_id)
	{
		...
		if ($GLOBALS['TYPO3_DB']->sql_num_rows($qry)) return 1;
		else return 0;
	}
	function Money2DutchString($input,$add_currency=1)
	{
		...
		if (is_numeric($input))
		{
			if ($add_currency) $content.=$this->ms['MODULES']['CURRENCY']." ";
	...
	function Money2PDFDutchString($input,$add_currency=1)
	{
		...
		if (is_numeric($input))
		{
			if ($add_currency)
			{
				if ($this->ms['MODULES']['CURRENCY_ARRAY']['cu_iso_3']=='EUR') 	$currency_char=chr(128);
				else														$currency_char=$this->ms['MODULES']['CURRENCY_ARRAY']['cu_symbol_left'];
	...
	*/
	...
	href=\"\\2\\3\" target=\"_blank\"><u>\\2\\3</u></a>\\4",$str); 
	} 
	function typolink ($page_id='', $vars='', $manual_link=0)
	{
		...
		elseif (!$page_id) $page_id=$GLOBALS["TSFE"]->id;
...
//		$conf['useCacheHash']=1; // dont use
		...
		if ($manual_link)
		{
			...
			else $url='index.php?id='.$page_id.$conf['additionalParams'];
		}
		else
		{
...
//			$mail->MsgHTML($body,$this->DOCUMENT_ROOT);
	...
	//		$mail->IsSendmail(); // telling the class to use SendMail transport
			if ($this->conf['email_tmpl_path'])  	$template = $this->cObj->fileResource($this->conf['email_tmpl_path']);
			else									$template = $this->cObj->fileResource(t3lib_extMgm::siteRelPath('multishop').'templates/email_template.tmpl');
			$markerArray=array();
			$markerArray['###BODY###'] = $content; 
			$body = $this->cObj->substituteMarkerArray($template, $markerArray);
...
//			$mail->MsgHTML($body,$this->DOCUMENT_ROOT);
	...
	function MsgHTMLwithEmbedImages(&$mail,$body) {
		...
		foreach ($matches[0] as $img) {
			...
			$arr = parse_url($m[1]);
			if ($arr['host']==$this->HTTP_HOST and (!isset($arr['host']) || !isset($arr['path'])))continue;
			// add
	...
	{
		global $SCRIPT_NAME;
		if ($subject)	mail('bas@bvbmedia.com',$subject,$SCRIPT_NAME.': '."\n".$input);
		else			mail('bas@bvbmedia.com',$SCRIPT_NAME,$input);
	}
		...
		if ($total_rows > 0) {
			...
			while ($options=$GLOBALS['TYPO3_DB']->sql_fetch_assoc($qry)) {
				...
				switch ($options['listtype']) {
					case 'divider':
						$output.='<div class="opties-field-attribute'.$options['products_options_id'].' opties-field-radio opties-field-divider">
						<label></label>
						<div class="hr"></div>
						</div>';
						$load_default=0;
					break;
					case 'input':
						$output.='<div class="opties-field-attribute'.$options['products_options_id'].' opties-field-radio opties-field-input">
				...
				if ($load_default) {
					...
					if (!$readonly) {
						$output.='<div class="'.$class.'"><label>'. $options['products_options_name'].':</label><div id="attribute_item_wrapper">';
					} else {
						$output.='<li><label>'.$options['products_options_name'].':</label> ';
					}
					
					// SHOW_ATTRIBUTE_DESCRIPTION
					if (SHOW_ATTRIBUTE_DESCRIPTION && !empty($products_options_name_values['description']))
					{
						$output.= $products_options_name_values['description']."<br/>";
					}
					$opt = 0;
	...
	function categoryHasSubs($categories_id)
	{
		if (is_numeric($categories_id))
		{
			...
			$qry=$GLOBALS['TYPO3_DB']->sql_query($str);
			if ($GLOBALS['TYPO3_DB']->sql_num_rows($qry) > 0) return true;
			else return false;
		}
	}
	...
	function getProduct($products_id,$categories_id='',$extra_fields='',$include_disabled_products=0,$skipFlatDatabase=0) {
		if ($skipFlatDatabase or (!$this->ms['MODULES']['FLAT_DATABASE'] or $include_disabled_products)) {
			...
			$select[]='oud.name as order_unit_label';

			if ($extra_fields) $select[]=$extra_fields;
	...
	function getNextPreviousProduct($products_id,$categories_id='')
	{
		...
		foreach ($pagination_items as $key => $item)
		{
			if ($item)
			{
				...
				$link=mslib_fe::typolink($this->shop_pid,'&'.$where.'&products_id='.$item.'&tx_multishop_pi1[page_section]=products_detail');			
	...
	*/
	function getProductsPageSet($filter=array(),$offset=0,$limit=0,$orderby=array(),$having=array(),$select=array(),$where=array(),$redirect_if_one_product=0,$extra_from=array(),$groupby=array(),$search_section='products_search',$select_total_count='',$returnTotalCountOnly=0,$enableFetchTaxRate=1) {
			...
			if (!$this->ms['MODULES']['FLAT_DATABASE']) {
					...
					if ($str_order_by)      $orderby_clause.=$str_order_by; 
			} else {
...
//       error_log($str);
//		 die();
			if ($returnTotalCountOnly) return $array['total_rows'];
...
//			die();
	...
	function getOptionsPageSet($filter=array(),$offset=0,$limit=0,$orderby=array(),$having=array(),$select=array(),$where=array()) {
		...
		$where_clause.=' and ';
		if (is_array($filter) and count($filter) >0) $where_clause.=implode(" and ",$filter)." ";
		elseif ($filter)	   $where_clause.=$filter." ";
	...
	*/
	function getCustomersPageSet($filter=array(),$offset=0,$limit=0,$orderby=array(),$having=array(),$select=array(),$where=array())
	{
		...
		if (is_array($filter) and count($filter) >0) $where_clause.=implode(" and ",$filter)." and ";
		elseif ($filter)	   $where_clause.=$filter." and ";
...
//		$row2=$GLOBALS['TYPO3_DB']->sql_fetch_row($qry2);
	...
	*/
	...
	{	
		...
		{
			case 'home':
				if ($this->ms['MODULES']['HOME_SPECIALS_BOX'])			require(t3lib_extMgm::extPath('multishop').'scripts/front_pages/products_specials.php');			
			break;
			case 'single_special':
...
*/					
	...
	*/	
	...
	function tx_multishop_get_category_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false, $exclude_not_active = false,$default_label='') {
		if (!$parent_id and $this->categoriesStartingPoint) $parent_id=$this->categoriesStartingPoint;
		if (!$default_label) $default_label=$this->pi_getLL('admin_main_category');
		
		if (!is_array($category_tree_array)) $category_tree_array = array();
		if ( (sizeof($category_tree_array) < 1) && ($exclude != '0') ) $category_tree_array[] = array('id' => $this->categoriesStartingPoint, 'text' => $default_label);
	...
	function hasCats ($categories_id,$include_disabled_categories=1) {
		...
		if (!$include_disabled_categories) $query.=" and status=1";
		$res = $GLOBALS['TYPO3_DB']->sql_query($query);
		$row=$GLOBALS['TYPO3_DB']->sql_fetch_assoc($res);
		if ($row['total']>0) return 1;
	}	
...
//				error_log(print_r($data_level,1));
	...
	function getUserGroupDiscount($uid)
	{
		...
		$discount=0;		
		if ($user['tx_multishop_discount']) $discount=$user['tx_multishop_discount'];
		if (!$discount)
		{
			if ($user['usergroup'])
			{
	...
	{
		$content=base64_decode(base64_decode('
')).$input;
		return $content;
	}
...
//		error_log($res);
	...
	{
		if (!$header_label and !$content)	return '';
		if ($this->conf['html_box_tmpl_path'])  				$template = $this->cObj->fileResource($this->conf['html_box_tmpl_path']);
		elseif ($this->conf['html_box_tmpl'])  					$template = $this->cObj->fileResource($this->conf['html_box_tmpl']);	
		else													$template = $this->cObj->fileResource(t3lib_extMgm::siteRelPath('multishop').'templates/html_box.tmpl');		
		...
		if (!$header_label)	$header_wrapper='';
		if (!$content) 		$content_wrapper='';
	...
	{
		$query = $GLOBALS['TYPO3_DB']->SELECTquery('o.*, od.name','tx_multishop_orders_status o, tx_multishop_orders_status_description od','o.default_status=1 and o.deleted=0 and od.language_id=\''.$language_id.'\' and o.page_uid=\''.$this->shop_pid.'\' and o.id=od.orders_status_id','','od.name','');	
		$res = $GLOBALS['TYPO3_DB']->sql_query($query);		
		$status=array();
		if ($GLOBALS['TYPO3_DB']->sql_num_rows($res) > 0)
		{
	...
	function getAllOrderStatus ($language_id=0) {
		$query = $GLOBALS['TYPO3_DB']->SELECTquery('o.*, od.name','tx_multishop_orders_status o, tx_multishop_orders_status_description od','od.language_id=\''.$language_id.'\' and (o.page_uid=\''.$this->showCatalogFromPage.'\' or o.page_uid=\'0\') and o.deleted=0 and o.id=od.orders_status_id','','od.name','');	
		$res = $GLOBALS['TYPO3_DB']->sql_query($query);
	...
	{
		if ($language_id=='') $language_id=$GLOBALS['TSFE']->sys_language_uid;
		$query = $GLOBALS['TYPO3_DB']->SELECTquery('od.name','tx_multishop_orders_status o, tx_multishop_orders_status_description od','o.id=\''.$id.'\' and od.language_id=\''.$language_id.'\' and o.page_uid=\''.$this->shop_pid.'\' and o.id=od.orders_status_id','','','');	
		$res = $GLOBALS['TYPO3_DB']->sql_query($query);
		if ($GLOBALS['TYPO3_DB']->sql_num_rows($res) > 0)
		{
			$row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res);
...
//						this.form.submit();
	...
	function jQueryAdminMenu() {
		static $ms_menu;
		if (is_array($ms_menu)) return $ms_menu;
		...
		if ($this->ROOTADMIN_USER or $this->CATALOGADMIN_USER)
		{
			...
			$ms_menu['header']['ms_admin_catalog']['subs']['ms_admin_products']['subs']['admin_search_and_edit_products']['link']=mslib_fe::typolink($this->shop_pid.',2003','tx_multishop_pi1[page_section]=admin_products_search_and_edit&cid='.$this->get['categories_id']);
...
//			$ms_menu['footer']['ms_admin_system']['subs']['admin_shipping_and_payment']['subs']['admin_tax_rules']['link']=mslib_fe::typolink($this->shop_pid,'tx_multishop_pi1[page_section]=admin_tax_rules');		
	...
	//	$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery($selectFields, $table, $whereClause);
		...
		if (is_array($languagesLabels) and count($languagesLabels)) {
			...
			<form action="'.mslib_fe::typolink().'" method="post" id="multishop_admin_language_form">
				...
				{
					if ($language['key']) 	$ms_menu['footer']['ms_admin_language']['description'].='<option value="'.$language['flag'].'"'.($this->cookie['multishop_admin_language']==$language['flag']?' selected':'').'>'.t3lib_div::strtoupper($language['value']).'</option>'."\n";
				}
				$ms_menu['footer']['ms_admin_language']['description'].='
				</select>
			</form>			
	...
	function getProductsOptionValues ($option_id, $products_id='') {
		if (is_numeric($option_id)) {
			...
			if (is_numeric($products_id)) $str.="pa.products_id='".$products_id."' and ";
	...
	function tep_get_categories_edit($categories_id='', $aid='')
	{
		...
		if ($categories_id) $parent_categories_query = $GLOBALS['TYPO3_DB']->sql_query("SELECT cd.categories_name as name, cd.categories_id as id, c.parent_id as parent from tx_multishop_categories c, tx_multishop_categories_description cd where c.parent_id='" . $categories_id . "' and c.page_uid='".$this->showCatalogFromPage."' and cd.language_id='".$GLOBALS['TSFE']->sys_language_uid."' and c.status=1 and c.categories_id=cd.categories_id order by c.sort_order, cd.categories_name");
		else $parent_categories_query = $GLOBALS['TYPO3_DB']->sql_query("SELECT cd.categories_name as name, cd.categories_id as id, c.parent_id as parent from tx_multishop_categories c, tx_multishop_categories_description cd where c.parent_id='0' and c.status=1 and c.page_uid='".$this->showCatalogFromPage."' and cd.language_id='".$GLOBALS['TSFE']->sys_language_uid."' and c.categories_id=cd.categories_id order by c.sort_order, cd.categories_name");
		...
		while ($parent_categories = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($parent_categories_query))
		{
			...
			if (!$GLOBALS['TYPO3_DB']->sql_num_rows($qrychk)) $html.=' <a href="'.mslib_fe::typolink(',2002','&tx_multishop_pi1[page_section]=admin_ajax&cid='.$parent_categories['id'].'&action=add_product').'" onclick="return hs.htmlExpand(this, { objectType: \'iframe\', width: 890, height: 500} )" class="admin_menu_add" title="Add Product"></a>';
			if (!$GLOBALS['TYPO3_DB']->sql_num_rows($qrychk)) $html.=' <a href="#" cid="'.$parent_categories['id'].'" class="admin_menu_upload_productfeed" title="Upload Productfeed"></a>';
			...
			if ($GLOBALS['TYPO3_DB']->sql_num_rows($qrychk)) $html .=mslib_fe::tep_get_categories_edit($parent_categories['id'],$aid);
			else
			{
	...
	function loadInherentCustomSettingsByProduct($products_id,$categories_id='')
	{
		...
		if ($product['custom_settings']) $settings[]=$product['custom_settings'];
		if (count($settings))
		{
		...
		if (is_numeric($uid))
		{
			...
			if ($user['uid'])
			{
				...
				foreach ($groups as $group)
				{
					...
					if ($group)
					{
						if (is_array($remove_usergroup) or $remove_usergroup)
						{
							...
							elseif ($group != $remove_usergroup) 											$new_groups[]=$group;
						}
						else
						{
							$new_groups[]=$group;
				...
				if (is_array($add_usergroup))
				{
					...
					{
						if ($item) $new_groups[] = $item;
					}
				}
				elseif($add_usergroup) $new_groups[] = $add_usergroup;				
	...
	function generateInvoiceId()
	{
		...
		else
		{
			...
			if (preg_match("/^".$prefix."/",$rs_inv['invoice_id'])) $invoice_id = ((int) $rs_inv['invoice_id'] + 1);
			else
			{
		...
		if (is_numeric($orders_id))
		{
			...
			if (($order['orders_id'] and $order['bill']) or ($order['orders_id'] and $force))
			{
				...
				if ($invoice_id)
				{
					...
					if ($order['billing_company']) 	$name=$order['billing_company'];
					else							$name=$order['billing_name'];
					...
					if ($res)
					{
						...
						if ($this->ms['MODULES']['LOCK_ORDER_AFTER_CREATING_INVOICE']) 	$lock_order=1;
						else														$lock_order=0;
	...
	function updateOrderStatusToPaid($orders_id)
	{
		...
		if (!$order['paid'])
		{
			...
			if ($this->ms['MODULES']['ORDERS_PAID_CUSTOM_SCRIPT'] and $continue)
			{
				if (!strstr($module_settings['ORDERS_PAID_CUSTOM_SCRIPT'],".."))
				{
					if (strstr($module_settings['ORDERS_PAID_CUSTOM_SCRIPT'],"/"))	require(PATH_site.$module_settings['ORDERS_PAID_CUSTOM_SCRIPT'].'.php');	
				}					
			}		
	...
	{
		return '
		<div class="fullwidth_div">'.$content.'</div>
		';
	}
	/*
	...
	*/
	...
	function getRecordsPageSet($data=array()) {
		if (is_array($data) and count($data) and count($data['from'])) {
			...
			if (!is_array($data['select'])) 	$data['select']=array('*');
			if (!is_array($data['where'])) 		$data['where']=array();
			if (!is_array($data['group_by'])) 	$data['group_by']=array();
			if (!is_array($data['order_by'])) 	$data['order_by']=array();
...
//								$where.='&';
						...
						$content.='
						<span class="products_name"><a href="'.$link.'" class="ajax_link" alt="'.$product['products_meta_description'].'" title="'.$product['products_meta_description'].'">'.$product['products_name'].'</a></span>
						';
