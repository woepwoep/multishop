...
if (mslib_fe::loggedin())
{
	...
	$user['gender'] = $user['gender'] == 0 ? 'm' : 'f';		
		...
		if ($this->method == 'edit_account'){
			$validate_password = '';
		} else {
			
			$validate_password = 'jQuery("#password").validate({
									expression: "if (VAL) return true; else return false;",
									message: "'.$this->pi_getLL('password_is_required').'"
							 });';
		}
			 ...
			 <script type="text/javascript">
					  ...
					  '.((count($enabled_countries) > 1)? '
						...
						jQuery().ready(function(){
							...
							jQuery("#delivery_birthday").datepicker({ 
															defaultDate: "+1w",
															dateFormat: "dd-mm-yy",
															changeMonth: true,
															changeYear: true,
		...
		<div class="account-field">
			...
			<input type="text" name="username" class="username" id="username" value="'.htmlspecialchars($user['username']).'"/>
		...
		<div class="step">
			<div class="account-field">
				...
				<input type="text" name="birthday" class="birthday" id="birthday" value="'.htmlspecialchars($user['date_of_birth']).'" >
				<span class="error-space"></span>	
			</div>
		</div>
		<div class="step">
		...
		<script>
			jQuery("#delivery_address").validate({
               ...
               jQuery("#delivery_telephone").validate({
                    expression: "if (!isNaN(VAL) && VAL && VAL.length == '.$this->ms['MODULES']['CHECKOUT_LENGTH_TELEPHONE_NUMBER'].') return true; else return false;",
                    message: "Telephone is a required field and must be '.$this->ms['MODULES']['CHECKOUT_LENGTH_TELEPHONE_NUMBER'].' digits long (delivery address)."
                });	
		</script>
		...
		$content.='	
				<div id="bottom-navigation">
						<div id="navigation"> 							
	 						...
	 						<input type="submit" id="submit" value="'.  ($this->method == 'edit_account' ? ucfirst($this->pi_getLL('update_account')) : ucfirst($this->pi_getLL('register'))) .'"/>
	 					</div>
				</div>
				</form>
				</div>
