...
foreach ($dates as $key => $value)
{
	...
	$where=array();	
	if ($this->cookie['no_checkout_cart_entries_only']) 	$where[]='(c.is_checkout=0)';
	else												$where[]='(c.is_checkout=0 or c.is_checkout=1)';
		...
		if (!in_array($row['session_id'],$session_ids))
		{
			...
			if (count($cart['products']) > 0)
			{
				...
				if (count($products) >0)
				{
					...
					if ($cart['user']['username']) 		$tr_rows[]='<th class="text_right" width="100">'.$this->pi_getLL('username').'</th><td>'.$cart['user']['username'].'</td>';
					if ($cart['user']['first_name'])	$tr_rows[]='<th class="text_right" width="100">'.$this->pi_getLL('name').'</th><td>'.$cart['user']['first_name'].' '.$cart['user']['middle_name'].' '.$cart['user']['last_name'].'</td>';
					if ($cart['user']['company']) 		$tr_rows[]='<th class="text_right" width="100">'.$this->pi_getLL('company').'</th><td>'.$cart['user']['company'].'</td>';
					if ($cart['user']['telephone']) 	$tr_rows[]='<th class="text_right" width="100">'.$this->pi_getLL('telephone').'</th><td>'.$cart['user']['telephone'].'</td>';
					if ($cart['user']['email']) 		$tr_rows[]='<th class="text_right" width="100">'.$this->pi_getLL('email').'</th><td>'.$cart['user']['email'].'</td>';
					...
					{
						if (!$sub_sub_tr_type or $sub_sub_tr_type=='even') 	$sub_sub_tr_type='odd';
						else												$sub_sub_tr_type='even';						
...
//					$tmp_content.='</table>';
					...
					{
						if (!$sub_tr_type or $sub_tr_type=='even') 	$sub_tr_type='odd';
						else										$sub_tr_type='even';
