...
<form action="'.mslib_fe::typolink($this->shop_pid,'tx_multishop_pi1[page_section]=admin_system_clear_database').'" method="post">
	<div class="account-field">
			...
			<ul>
				<li><input name="tx_multishop_pi1[items][]" type="checkbox" value="categories" /> categories</li>
				<li><input name="tx_multishop_pi1[items][]" type="checkbox" value="products" /> products</li>
				<li><input name="tx_multishop_pi1[items][]" type="checkbox" value="everything" /> everything</li>
			</ul>
	</div>
	<div class="account-field">
			<label></label>
		...
		switch ($item)
		{
			...
			case 'everything':
				...
				*/
				...
				{
					if ($item)	$qry = $GLOBALS['TYPO3_DB']->sql_query($item);
				}
