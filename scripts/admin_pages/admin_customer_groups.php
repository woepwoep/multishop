...
{
	$erno=array();
	if (!$this->post['group_name']) 			$erno[]='Group name is not defined';
	if (!is_numeric($this->post['discount'])) $this->post['discount']=0;
	if (!count($erno))
	{
		$insertArray=array();
...
if ($p >0) $offset=(((($p)*$this->ms['MODULES']['ADMIN_CUSTOMERS_LISTING_LIMIT'])));
else 
{
	$p=0;
	$offset=0;
...
<form id="form1" name="form1" method="post" action="'.mslib_fe::typolink($this->shop_pid,'tx_multishop_pi1[page_section]=admin_customer_groups').'">	
...
//if (isset($this->get['tx_multishop_pi1']['do_search']))
//{	
	// product search
	$filter		=array();
	$having		=array();
