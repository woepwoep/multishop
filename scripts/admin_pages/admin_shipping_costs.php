		...
		if ($shipping_type == 'weight')
		{
			...
			{			
				foreach ($this->post as $key => $weight)
				{
					...
					} //akhir pengecekan untuk memasukan nilau flat
					unset($insertArray);
				}			
		}
...
if (count($shipping_methods) > 0)
{
	...
	foreach ($shipping_methods as $row)
	{
		...
		if ($row['shipping_costs_type']=='flat'){
			foreach ($zones as $zone) // start for flat based
			{
				$content.='<fieldset>';
...
else $content.='Currently there isn\'t any shipping method defined.';
